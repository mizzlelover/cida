...
- 不做"全文一个强度"的输出（见修辞密度曲线，`STYLE_SYSTEM.md` §4）。
...
knowledge/sources/           来源登记处（Source Registry，可追溯）
knowledge/practitioner_hypotheses/  实践者假说登记处（经验≠军规）
...
- 用户经验一律登记为 Practitioner Hypothesis（`knowledge/practitioner_hypotheses/`），
