...
claim: ""           # 断言
evidence: ""        # 证据
source: ""          # → 来源登记处（随开发仓分发）的 source_id
counterevidence: "" # 反证
boundary: ""        # 边界条件
...
- `metadata_only` 只能进候选池，不得支撑任何知识结论（研究侧语料档案随开发仓分发，不在发布包内）；
...
> 本节工具与档案随开发仓分发，使用发布包写作时无需运行。

- `python scripts/audit_research.py`：七类审计 + 自动生成研究侧覆盖率报告
  （覆盖率按 Found/Acquired/Read/Analyzed/Accepted 分列）；
