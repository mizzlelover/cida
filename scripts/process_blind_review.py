...
本脚本机械完成 BLIND_REVIEW_PACK §6 的"揭盲-汇总"段：
  ...
  2. 按揭盲钥（统一奇偶规则 + pair_001 例外）把 A/B 映射为 Skill/Baseline；
...
def skill_side(pair_id: str) -> str:
    """统一揭盲规则：偶数 pair A=Skill；奇数 pair B=Skill；pair_001 例外 B=Skill。"""
    no = int(pair_id.replace("pair_", ""))
    if no == 1:
        return "B"
    return "A" if no % 2 == 0 else "B"
...
def main():
    ...
    for q in PAIR_COLS:
        sk = sum(1 for r in reviewed if (r.get(q) or "").strip() == skill_side(r["pair_id"].strip()))
        bl = sum(1 for r in reviewed if (r.get(q) or "").strip() and (r.get(q) or "").strip() != "平"
                 and (r.get(q) or "").strip() != skill_side(r["pair_id"].strip()))
