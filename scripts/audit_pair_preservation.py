...
- 揭盲映射取自 `pending_pairs/keys_*.md`（经 `blind_keys.py`，与 process_blind_review 同源）：
  区分"设计性缺失"（Baseline 侧）与"Skill 侧需回溯修复"完全依赖该映射。
  **2026-09-16 勘误**：此前缺省用"偶数 pair A=Skill、奇数 pair B=Skill"的奇偶公式，而
  pair_012/014/016/018/020 的实际 A 侧是 Baseline——公式会把这五对的缺失**归错侧**。
  现改为读 keys 文件；配对无钥时报错列出，不猜。
...
def main():
    ...
    keys = load_keys(Path(args.pairs_dir))
    ...
    for f in files:
        ...
        skill = keys.get(no)
        if skill is None:
            report.append({"pair": base, "error": "no_unmask_key（keys_*.md 未登记）"})
            continue
