...
- **可追溯性补写（2026-09-16）**：本节此前只以**目录形式**引用（`corpus/blogs/`、`corpus/interviews/`、`corpus/hosting/`），**自动化计数读不到具体条目**（与 `information_density` 曾修的是同一类缺陷）。现补齐 5 位表达者的显式 `corpus_id`——阮一峰 `corpus.blogs.ruanyifeng-weekly-406`、`corpus.blogs.ruanyifeng-essay-survivor-preface`；陈皓 `corpus.blogs.coolshell-22298`、`corpus.blogs.coolshell-22341`；云风 `corpus.blogs.codingnow-rftg-strategy`；和菜头 `corpus.blogs.hecaitou-the-groundwork-for-romance`；刘未鹏 `corpus.blogs.mindhacks-eternal-golden-diagonal`。**仍为目录级、未落实具体条目**：主持/访谈语料（陈鲁豫／王志／水均益／柴静／鲁健）——该批条目为 `READ` 级，只作**结构级边界比较**，**不计入本节点的表达者样本数**。
