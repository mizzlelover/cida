...
├── knowledge/                ← 知识库（机制库已成；领域库在建，现为要点 README）
│   ├── register/                 语域行为清单（口表约束·在建）
│   ├── mechanisms/               41 个话语机制节点（已成）
│   ├── anti_patterns/            反模式库（已成）
│   ├── rhetoric/                 修辞学（在建·要点）
│   ├── pragmatics/               语用学（在建·要点）
│   ├── discourse/                篇章语言学（在建·要点）
│   ├── conversation/             会话分析（在建·要点）
│   ├── hosting/                  主持人点评与现场表达（在建·要点）
│   └── writing/                  写作传统（在建·要点）
├── platforms/                ← 7 个平台适配器
├── schemas/                  ← YAML Schema
...
<img src="docs/assets/wechat-qr-square.png?v=2" width="280" alt="微信搜一搜「谁是专家」">
