...
3. 从 `knowledge/mechanisms/GRAPH.md` 取与平台任务对应的 `audience_design`、`topic_opening`、`information_density` 或 `closure` 节点，只改呈现层动作；
4. 按平台参数调整：段落长度、小标题密度、开头速度、语域偏移、排版约定；
5. 用 `STYLE_SYSTEM.md` 复核十维坐标，对照 `knowledge/anti_patterns/README.md` 索引检查是否滑入平台反模式；
