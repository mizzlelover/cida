...
- 诊断前读取 `knowledge/anti_patterns/`，只选择与文本症状相符的反模式；
- 依据问题所在段落，从 `knowledge/mechanisms/GRAPH.yaml` 取对应机制节点，连同“边界、滥用风险、修复动作”一起读取；
- 语域和节奏参数以 `STYLE_SYSTEM.md` 为坐标，必要时用 `corpus/contrast/` 做反向校准；
- 口述来源必须对照 `corpus/raw_edited_pairs/`，主持/访谈来源只借鉴任务机制，不复制个人句式；
